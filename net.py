...
class DCFNet(nn.Module):
    ...
    def forward(self, x):
        ...
        response = torch.fft.irfft(complex_mul(kxzf, self.model_alphaf),2)

        return response

    def update(self, z, lr=1.):
        z = self.feature(z) * self.config.cos_window
        zf = torch.fft.rfft(z,2)
        kzzf = torch.sum(torch.sum(zf ** 2))
...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load('/home/airlab/PycharmProjects/pythonProject5/UDT_pytorch/track/param.pth'))
